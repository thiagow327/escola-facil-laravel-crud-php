...
@section('content')
    <main class="container">
        <section>
            <form action="{{ route('alunos.update', $aluno->id) }}" enctype="multipart/form-data" method="post">
                @csrf
                @method('PUT')
                <div class="titlebar">
                    <h1>Editar Aluno</h1>
                ...
                <div class="card">
                    <div class="bloco-aluno">
                        ...
                        <textarea cols="15" rows="3" name="observacao_de_saude"
                            placeholder="Ex: Sofre de asma, manter medicamento de alívio rápido disponível.">{{ $aluno->observacao_de_saude }}</textarea>
                        <hr>
                        ...
                        <select name="escola">
                            @foreach (json_decode('{"EMEI PROF. TITO LÍVIO FERREIRA":"EMEI PROF. TITO LÍVIO FERREIRA","EMEF PROF. RAUL FERNANDES":"EMEF PROF. RAUL FERNANDES"}', true) as $optionKey => $optionValue)
                                <option value="{{ $optionKey }}" {{ $optionKey == $aluno->escola ? 'selected' : '' }}>
                                    {{ $optionValue }}</option>
                            @endforeach
                        </select>
                        ...
                        <select name="turno">
                            @foreach (json_decode('{"MANHÃ":"MANHÃ","TARDE":"TARDE"}', true) as $optionKey => $optionValue)
                                <option value="{{ $optionKey }}" {{ $optionKey == $aluno->turno ? 'selected' : '' }}>
                                    {{ $optionValue }}</option>
                            @endforeach
                        </select>
                        ...
                        <img src="{{ asset('fotos/' . $aluno->foto) }}" alt="" class="img-student"
                            id="file-preview" />
                    ...
                    <div class="bloco-responsaveis">
                        ...
                        <input type="text" name="valor_da_mensalidade" value="{{ $aluno->valor_da_mensalidade }}">
                    </div>
